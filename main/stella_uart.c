...
int extract_info_RS9A(char *ver_str, char *sn_str, char *value_str)
{
	char* str1 = (char*)0;
	char* str2 = (char*)0;
	char* token = (char*)0;
...
//          .source_clk = UART_SCLK_DEFAULT,
    ...
    while (1) {
		...
		loop_count_rs9a %= 3 ; 
		ESP_LOGW("shcho_test", "\n>>>uart write: %s", Query_str_RS9A[loop_count_rs9a]);
            ...
            if (s < 0) {
                ...
                if (FD_ISSET(fd_uart1, &rfds)) 
				{
                    ...
                    if (len_read > 0)
					{
						...
						{
							extract_info_RS9A(buf_str0, buf_str1, buf_str2);
						} 
						break;
                    } else {
...
//  
    ...
    xTaskCreate(uart_select_task_uart1, "uart_select_task_uart1", 4 * 1024, NULL, 5, NULL);
}
